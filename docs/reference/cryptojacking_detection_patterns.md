...
**핵심 통찰**: KT cloud + INESC-ID 가 공통으로 강조 — **"사용자 inactive 인데 자원만 풀가동"** 이 mining 의 거의 확정적 시그니처. RADA 는 `client_core/collector/system_idle.py` 에서 Windows `GetLastInputInfo` 로 `user_idle_ms` 를 수집하며, S1 패턴이 이를 사용해 발화한다 (`ml_server/scorer/pattern_categories.py` §S1). Grafana 시각화는 향후 패널 추가 예정.
...
### 8.1 추가 수집 필요 (client_core) — 현재 상태

| 패턴 | 필요 데이터 | 수집 상태 | ML 평가 |
|---|---|---|---|
| **S1** | `user_idle_ms` (GetLastInputInfo) | ✅ `client_core/collector/system_idle.py` | ✅ `pattern_categories.py:S1` |
| S2 | `screen_locked` boolean | ❌ 미수집 | ⏸ stub |
| **S3** | `disk_io_mb_per_s` (read+write) | ✅ 기존 수집 | ✅ `pattern_categories.py:S3` |
| **S4** | `mem_used_gb` | ✅ 기존 수집 | ✅ `pattern_categories.py:S4` |
| S5 | process recreation 추적 | ❌ 미수집 | ⏸ stub |
| R5 | `gpu_power_w` | ✓ derived_features 에 일부 있음 (gpu.power_draw_w) | (보조 신호) |
| R6 | `tensor_core_active` vs SM | ⚠ tensor_core_active 키 존재하지만 항상 None (NVML 한계) | ⏸ stub |
| R9 | per-core CPU breakdown | ❌ 현재 평균만 수집 | ⏸ stub |
| N1 | per-packet 크기 + 주기 | ❌ 현재 총량만 수집 | ⏸ stub |
| N6 | DNS resolution 새 외부 도메인 entropy | ❌ DNS 로그 미수집 | ⏸ stub |
| T2, T4 | 시간대 / 요일 | ✓ timeslot 변수 존재 | (context_discount) |

**구현 완료 패턴 요약** (`ml_server/scorer/pattern_categories.py` line 9 주석 기준):
- Resource: R1, R2, R3, R4, R5, R7, R8 ✅
- Network: N2, N4, N5 ✅
- System: **S1**, S3, S4 ✅
- 보류 (stub): R6, R9, N1, N6, S2, S5
